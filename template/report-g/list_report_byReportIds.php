    ...
    foreach ($result_group as $row_group)
    {
        ...
        if ($report_count > 0)
        {
            foreach ($result as $row)
            {
                ...
                <td>" . $row['reasons'] . ($row['reason_custom'] != '' ? (' + ' . $row['reason_custom']) : '') . "</td>\n
                <td>%s</td>\n
            </tr>",
                    $row["report_id"],
                    $row["time_create"],
                    ...
                    $row["map_id"], sprintf("http://ezpz.cz/ext/phpbb/pages/styles/pbtech/template/utils-gotv/download.php?id=%d&file=%s/%s", $row["server_id"], $row["path"], $row["demo_file"]), $row["map"], $row["round"],
                    $row["note"]
                );
            }
